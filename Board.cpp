...
void Board::addToBoard(int x, int y, std::shared_ptr<Piece> piece) {
    int arrayPos = y * sizeParams::BOARD_SIZE + x;
...
void Board::renderPieces(SDL_Renderer *renderer, std::shared_ptr<Piece> &selectedPiece) {
    for(auto &piece : boardArray) {
        if (piece) {
            piece->render(renderer);
            if(piece == selectedPiece) {
                graphicallySelect(renderer, selectedPiece);
            }
        }
    }
}
...
void Board::graphicallySelect(SDL_Renderer *renderer, std::shared_ptr<Piece> &selectedPiece, ) {
    int x1 = selectedPiece->getSdl_rect().x;
    int y1 = selectedPiece->getSdl_rect().y;
    selectionRect.x = x1;
    selectionRect.y = y1;
    SDL_RenderCopy(renderer, Game::textureMap[selectionTexture]->getSDLTexture(), NULL, &selectionRect);
}